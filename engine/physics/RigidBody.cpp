...
void RigidBody::solveCollision(RigidBody::CollisionInfo info) {
    CollisionPoint p = info.point;

    Vec3D velocity_perpendicular = p.normal * velocity().dot(p.normal);
    Vec3D velocity_parallel = velocity() - velocity_perpendicular;

    setVelocity(Vec3D(0, 0, velocity().z()));
}
