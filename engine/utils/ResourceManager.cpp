...
std::vector<std::shared_ptr<Mesh>> ResourceManager::loadObjects(const std::string &filename, const ObjectNameTag& nameTag) {
    ...
    if (it != _instance->_objects.end()) {
        std::vector<std::shared_ptr<Mesh>> tmpObjects;
        for(auto &mesh : it->second) {
            tmpObjects.push_back(std::make_shared<Mesh>(ObjectNameTag(nameTag.str() + "_temp_obj_" + std::to_string(tmpObjects.size())), mesh->triangles()));
        }
        return tmpObjects;
    }
    ...
    while (!file.eof()) {
        ...
        if (line[0] == 'o') {
            if (!tris.empty())
                objects.push_back(
                        std::make_shared<Mesh>(ObjectNameTag(nameTag.str() + "_temp_obj_" + std::to_string(objects.size())), tris));
            tris.clear();
        }
        if (line[0] == 'v') {
            double x, y, z;
